 ...
 * under the License.
 */
package io.ballerina.observe.trace.opentelemetry;

import io.ballerina.observe.trace.opentelemetry.sampler.RateLimitingSampler;
...
/**
 * This is the Opentelemetry tracing extension class for {@link TracerProvider}.
 */
public class OpentelemetryTracerProvider implements TracerProvider {
    private static final String TRACER_NAME = "opentelemetry";
    ...
    public static void initializeConfigurations(BString reporterEndpoint, BMap<BString, BString> headers,
        ...
        OtlpGrpcSpanExporterBuilder exporterBuilder = OtlpGrpcSpanExporter.builder().
                setEndpoint(String.valueOf(reporterEndpoint));

        for (BString key : headers.getKeys()) {
            exporterBuilder.addHeader(String.valueOf(key), String.valueOf(headers.get(key)));
        }
        ...
        console.println("ballerina: started publishing traces in Opentelemetry format to API on " + reporterEndpoint);
    }
